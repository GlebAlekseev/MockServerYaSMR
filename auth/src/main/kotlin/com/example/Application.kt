...
fun checkEnv() {
    ...
    val clientSecret =
        System.getenv()["YANDEX_CLIENT_SECRET"] ?: throw RuntimeException("ENV YANDEX_CLIENT_SECRET is not exist")
    val yandexCallback =
        System.getenv()["YANDEX_CALLBACK"] ?: throw RuntimeException("ENV YANDEX_CALLBACK is not exist")
    println(
        "Получены переменные окружения: " +
                "\n\tport=$port" +
                "\n\tmongoUrl=$mongoUrl" +
                "\n\tapiUrl=$apiUrl" +
                "\n\tclientId=$clientId" +
